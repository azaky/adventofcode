...
let solve adj_list =
  ...
  in
  let find_ways source sink =
    let ways = Hashtbl.create (module String) ~size:n in
    let rec dfs node =
      if String.equal node sink
      then 1
    ...
    dfs source
  in
  (find_ways "svr" "fft" * find_ways "fft" "dac" * find_ways "dac" "out")
  + (find_ways "svr" "dac" * find_ways "dac" "fft" * find_ways "fft" "out")
